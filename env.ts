 ...
 *
 * file to load environement files
 * .env, .env.local, .env.{NODE_ENV} .env.{NODE_ENV}.local
 *
 * Variables will not wil loaded if the not defined at the end `const env = getEnv({...})`
 */
...
type DefinedENV = {
  [K in keyof typeof env]: string;
};

declare global {
  // eslint-disable-next-line @typescript-eslint/no-namespace
  namespace NodeJS {
    interface ProcessEnv extends DefinedENV {}
  }
}
...
function getEnv<T extends Record<string, unknown>>(map: T) {
  ...
  for (const key in map) {
    const option = (typeof map[key] === 'object' ? map[key] : { default: map[key] }) as {
      default?: unknown;
      required?: boolean;
    };

    const value = raw[key] || process.env[key] || '';
